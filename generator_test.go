...
var (
	_muted   = []int{244, 244, 245}
	_primary = []int{251, 187, 24}
)

func TestMIW(t *testing.T) {
	doc, err := New(Invoice, &Options{
		CurrencySymbol:        "R",
		CurrencyThousand:      ",",
		DarkBgColor:           _primary,
		GreyBgColor:           _muted,
		HeaderBgColor:         _primary,
		TextVersionTitle:      "VAT Number",
		TextRefTitle:          "Order Number",
		TextItemsTotalHTTitle: "Total excl. VAT",
		TextItemsTaxTitle:     "VAT",
		TextTotalTax:          "Total VAT",
		TextTotalWithTax:      "Total with VAT",
		TextItemsTaxSubscript: false,
		TextPaymentTermTitle:  "Payment Status",
	})
	...
	doc.SetHeader(&HeaderFooter{
		Text:       "<center>Made in Workshop</center>",
		Pagination: true,
	})
	...
	doc.SetRef(fmt.Sprintf("%06d", 36))
	doc.SetVersion("VAT123456789")
